...
function createChannel() {
  ...
  scripts/peerAdd.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE $bft_true
}
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/ordererOrg2/msp organizations/fabric-ca/ordererOrg2/tls-cert.pem organizations/fabric-ca/ordererOrg2/ca-cert.pem organizations/fabric-ca/ordererOrg2/IssuerPublicKey organizations/fabric-ca/ordererOrg2/IssuerRevocationPublicKey organizations/fabric-ca/ordererOrg2/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/ordererOrg3/msp organizations/fabric-ca/ordererOrg3/tls-cert.pem organizations/fabric-ca/ordererOrg3/ca-cert.pem organizations/fabric-ca/ordererOrg3/IssuerPublicKey organizations/fabric-ca/ordererOrg3/IssuerRevocationPublicKey organizations/fabric-ca/ordererOrg3/fabric-ca-server.db'
    ...
    infoln "Removing remaining compose docker volume"
    docker volume rm compose_orderer.example.com compose_orderer2.example.com compose_orderer3.example.com compose_peer0.org1.example.com compose_peer0.org2.example.com compose_peer1.org1.example.com compose_peer1.org2.example.com 
    docker volume prune
  fi
}
