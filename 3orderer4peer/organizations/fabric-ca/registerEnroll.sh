...
function createOrderer2() {
  infoln "Enrolling the CA admin2"
  mkdir -p organizations/ordererOrganizations/ord2.example.com

  export FABRIC_CA_CLIENT_HOME=${PWD}/organizations/ordererOrganizations/ord2.example.com

  set -x
  fabric-ca-client enroll -u https://admin:adminpw@localhost:29054 --caname ca-orderer2 --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg2/ca-cert.pem"
  { set +x; } 2>/dev/null

  echo 'NodeOUs:
  Enable: true
  ClientOUIdentifier:
    Certificate: cacerts/localhost-29054-ca-orderer2.pem
    OrganizationalUnitIdentifier: client
  PeerOUIdentifier:
    Certificate: cacerts/localhost-29054-ca-orderer2.pem
    OrganizationalUnitIdentifier: peer
  ...
  OrdererOUIdentifier:
    Certificate: cacerts/localhost-29054-ca-orderer2.pem
    OrganizationalUnitIdentifier: orderer' > "${PWD}/organizations/ordererOrganizations/ord2.example.com/msp/config.yaml"

  # Since the CA serves as both the organization CA and TLS CA, copy the org's root cert that was generated by CA startup into the org level ca and tlsca directories

  # Copy orderer org's CA cert to orderer org's /msp/tlscacerts directory (for use in the channel MSP definition)
  mkdir -p "${PWD}/organizations/ordererOrganizations/ord2.example.com/msp/tlscacerts"
  cp "${PWD}/organizations/fabric-ca/ordererOrg2/ca-cert.pem" "${PWD}/organizations/ordererOrganizations/ord2.example.com/msp/tlscacerts/tlsca.example.com-cert.pem"

  # Copy orderer org's CA cert to orderer org's /tlsca directory (for use by clients)
  mkdir -p "${PWD}/organizations/ordererOrganizations/ord2.example.com/tlsca"
  cp "${PWD}/organizations/fabric-ca/ordererOrg2/ca-cert.pem" "${PWD}/organizations/ordererOrganizations/ord2.example.com/tlsca/tlsca.example.com-cert.pem"

  infoln "Registering orderer2"
  set -x
  fabric-ca-client register --caname ca-orderer2 --id.name orderer2 --id.secret ordererpw --id.type orderer --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg2/ca-cert.pem" --url https://localhost:29054
  { set +x; } 2>/dev/null

  infoln "Registering the orderer2 admin"
  set -x
  fabric-ca-client register --caname ca-orderer2 --id.name orderer2Admin --id.secret ordererAdminpw --id.type admin --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg2/ca-cert.pem" --url https://localhost:29054
  { set +x; } 2>/dev/null

  infoln "Generating the orderer2 msp"
  set -x
  fabric-ca-client enroll -u https://orderer2:ordererpw@localhost:29054 --caname ca-orderer2 -M "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer2.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg2/ca-cert.pem"
  { set +x; } 2>/dev/null

  cp "${PWD}/organizations/ordererOrganizations/ord2.example.com/msp/config.yaml" "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer2.example.com/msp/config.yaml"

  infoln "Generating the orderer-tls certificates, use --csr.hosts to specify Subject Alternative Names"
  set -x
  fabric-ca-client enroll -u https://orderer2:ordererpw@localhost:29054 --caname ca-orderer2 -M "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer2.example.com/tls" --enrollment.profile tls --csr.hosts orderer2.example.com --csr.hosts localhost --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg2/ca-cert.pem"
  { set +x; } 2>/dev/null
  ...
  infoln "Generating the admin msp"
  set -x
  fabric-ca-client enroll -u https://orderer2Admin:ordererAdminpw@localhost:29054 --caname ca-orderer2 -M "${PWD}/organizations/ordererOrganizations/ord2.example.com/users/Admin@example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg2/ca-cert.pem" 
  { set +x; } 2>/dev/null

  cp "${PWD}/organizations/ordererOrganizations/ord2.example.com/msp/config.yaml" "${PWD}/organizations/ordererOrganizations/ord2.example.com/users/Admin@example.com/msp/config.yaml"
}


function createOrderer3() {
  infoln "Enrolling the CA admin3"
  mkdir -p organizations/ordererOrganizations/ord3.example.com

  export FABRIC_CA_CLIENT_HOME=${PWD}/organizations/ordererOrganizations/ord3.example.com

  set -x
  fabric-ca-client enroll -u https://admin:adminpw@localhost:29154 --caname ca-orderer3 --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg3/ca-cert.pem"
  { set +x; } 2>/dev/null

  echo 'NodeOUs:
  Enable: true
  ClientOUIdentifier:
    Certificate: cacerts/localhost-29154-ca-orderer3.pem
    OrganizationalUnitIdentifier: client
  PeerOUIdentifier:
    Certificate: cacerts/localhost-29154-ca-orderer3.pem
    OrganizationalUnitIdentifier: peer
  ...
  OrdererOUIdentifier:
    Certificate: cacerts/localhost-29154-ca-orderer3.pem
    OrganizationalUnitIdentifier: orderer' > "${PWD}/organizations/ordererOrganizations/ord3.example.com/msp/config.yaml"

  # Since the CA serves as both the organization CA and TLS CA, copy the org's root cert that was generated by CA startup into the org level ca and tlsca directories

  # Copy orderer org's CA cert to orderer org's /msp/tlscacerts directory (for use in the channel MSP definition)
  mkdir -p "${PWD}/organizations/ordererOrganizations/ord3.example.com/msp/tlscacerts"
  cp "${PWD}/organizations/fabric-ca/ordererOrg3/ca-cert.pem" "${PWD}/organizations/ordererOrganizations/ord3.example.com/msp/tlscacerts/tlsca.example.com-cert.pem"

  # Copy orderer org's CA cert to orderer org's /tlsca directory (for use by clients)
  mkdir -p "${PWD}/organizations/ordererOrganizations/ord3.example.com/tlsca"
  cp "${PWD}/organizations/fabric-ca/ordererOrg3/ca-cert.pem" "${PWD}/organizations/ordererOrganizations/ord3.example.com/tlsca/tlsca.example.com-cert.pem"

  infoln "Registering orderer3"
  set -x
  fabric-ca-client register --caname ca-orderer3 --id.name orderer3 --id.secret ordererpw --id.type orderer --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg3/ca-cert.pem" --url https://localhost:29154
  { set +x; } 2>/dev/null

  infoln "Registering the orderer3 admin"
  set -x
  fabric-ca-client register --caname ca-orderer3 --id.name orderer3Admin --id.secret ordererAdminpw --id.type admin --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg3/ca-cert.pem" --url https://localhost:29154
  { set +x; } 2>/dev/null

  infoln "Generating the orderer3 msp"
  set -x
  fabric-ca-client enroll -u https://orderer3:ordererpw@localhost:29154 --caname ca-orderer3 -M "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer3.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg3/ca-cert.pem"
  { set +x; } 2>/dev/null

  cp "${PWD}/organizations/ordererOrganizations/ord3.example.com/msp/config.yaml" "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer3.example.com/msp/config.yaml"

  infoln "Generating the orderer-tls certificates, use --csr.hosts to specify Subject Alternative Names"
  set -x
  fabric-ca-client enroll -u https://orderer3:ordererpw@localhost:29154 --caname ca-orderer3 -M "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer3.example.com/tls" --enrollment.profile tls --csr.hosts orderer3.example.com --csr.hosts localhost --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg3/ca-cert.pem"
  { set +x; } 2>/dev/null
  ...
  infoln "Generating the admin msp"
  set -x
  fabric-ca-client enroll -u https://orderer3Admin:ordererAdminpw@localhost:29154 --caname ca-orderer3 -M "${PWD}/organizations/ordererOrganizations/ord3.example.com/users/Admin@example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/ordererOrg3/ca-cert.pem" 
  { set +x; } 2>/dev/null

  cp "${PWD}/organizations/ordererOrganizations/ord3.example.com/msp/config.yaml" "${PWD}/organizations/ordererOrganizations/ord3.example.com/users/Admin@example.com/msp/config.yaml"
}
